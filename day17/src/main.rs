...
type World = HashSet<(i64, i64, i64)>;

fn num_active_neighbors(x: i64, y: i64, z: i64, world: &World) -> usize {
    let mut count = 0;
    for x2 in (x - 1)..=(x + 1) {
        for y2 in (y - 1)..=(y + 1) {
            for z2 in (z - 1)..=(z + 1) {
    ...
    let mut world: World = INPUT
        ...
        .map(|(x, line)| {
            line.bytes()
                ...
                .map(move |(y, _byte)| (x as i64, y as i64, 0))
        })
        .flatten()
        .inspect(|a| println!("{:?}", a))
        .collect();
